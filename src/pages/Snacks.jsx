...
import SnacksCard from '../components/SnacksCard'
import { getSnacks } from '../api/api'
import { Loader, Loader2, TriangleAlert } from 'lucide-react'

const Snacks = () => {
    const [products, setSnacks] = useState(null)
        ...
        <>
            <div className='w-screen h-full flex justify-start items-start flex-row flex-wrap mt-14 mb-12 gap-y-20 gap-x-2'>
                ...
                {products.map((product, index) => (
                    <SnacksCard img={product.img} name={product.name} price={product.price} key={product._id} />
                ))
                }
            </div>
        </>
