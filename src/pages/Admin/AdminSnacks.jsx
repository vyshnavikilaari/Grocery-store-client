...
import { getSnacks, addSnacks, deleteSnacks, editSnacks } from '../../api/api'
...
const AdminSnacks = () => {
  ...
  //new
  const [currentSnacks, setCurrentSnacks] = useState(null)
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const response = await addSnacks(snacks)
  ...
  const editHelper = (snacks) => {
    console.log(snacks)
    setCurrentSnacks(snacks)
    setShowEdit(true)
  }
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editSnacks(snacks, currentSnacks._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteSnacks(id)
      if (response.status === 200) {
        ...
        <tbody>

          {
            snacks.map((snacks, index) => (
              <tr key={index}>
                <td className='p-4'>{snacks._id} </td>
                <td className='flex justify-start px-4 items-center'><img src={snacks.img} alt={snacks.title} className='h-12 w-12 object-cover rounded-full shadow-md bg-rose-500' /></td>
                <td className='p-4'>{snacks.title} </td>
                <td className='p-4'>{snacks.price}</td>
                <td className='p-4'>{snacks.quantity}</td>
               ...
               hover:bg-red-500 hover:text-white hover:shadow-red-500'
                    onClick={() => { handleDelete(snacks._id) }}>
                    <Trash />
                  </button>
                </td>
              </tr>
            ...
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  <h1 className='w-1/2 text-left text-xl mt-12 pt-20 font-bold text-green-500'>Add Snack</h1>
        ...
        <>
          <div className="absolute top-0 left-0 z-50 h-screen w-screen flex justify-center items-center bg-black/40 ">
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  <h1 className='w-1/2 text-left text-xl mt-12 pt-20 font-bold text-blue-500'>Edit Snack</h1>
                ...
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-8' onSubmit={handleEdit}>
                  <input ref={titleRef} type="text" name="" id="title" placeholder='Title' defaultValue={currentSnacks.title} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required autoFocus />
                  <input ref={imgRef} type="text" name="" id="img" placeholder='Image URL' defaultValue={currentSnacks.img} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={priceRef} type="number" name="" id="price" placeholder='Price' defaultValue={currentSnacks.price} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={quantityRef} type="text" name="" id="quantity" placeholder='Quantity' defaultValue={currentSnacks.price} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
