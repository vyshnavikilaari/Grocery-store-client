...
const AdminProducts = () => {
  ...
  const handleDelete = async (title) => {
    try {
      const response = await deleteProduct(title)
      if (response.status === 200) {
...

          {
            products.map((product, index) => (
              <tr key={index}>
                ...
                <td className='p-4'>{product.offer}</td>
               ...
               hover:bg-red-500 hover:text-white hover:shadow-red-500'
                    onClick={() => { handleDelete(product._title) }}>
                    <Trash />
                  </button>
                </td>
              </tr>
            ...
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  <h1 className='w-1/2 text-left text-xl mt-12 pt-20 font-bold text-green-500'>Add Product</h1>
                ...
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-8' onSubmit={handleAdd}>
                  ...
                  <input ref={offerRef} type="text" name="" id="offer" placeholder='Offer' className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-green-400 rounded-sm' required />
        ...
        <>
          <div className="absolute top-0 left-0 z-50 h-screen w-screen flex justify-center items-center bg-black/40 ">
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  <h1 className='w-1/2 text-left text-xl mt-12 pt-20 font-bold text-blue-500'>Edit Product</h1>
                ...
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-8' onSubmit={handleEdit}>
                  ...
                  <input ref={offerRef} type="text" name="" id="offer" placeholder='Offer' defaultValue={currentProduct.offer} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
