...
import { getFruitsandveg, addFruitsandveg, deleteFruitsandveg, editFruitsandveg } from '../../api/api'
import { toast } from 'sonner'

const AdminFruitsandveg = () => {
  //null -> products[] | Store the data
  const [fruitsandveg, setFruitsandveg] = useState(null)
  //true (shows loading screen) -> false(hide loading screen) | Condition Render
  const [loading, setLoading] = useState(true)
  //new
  const [currentFruitsandveg, setCurrentFruitsandveg] = useState(null)
  ...
  const handleAdd = async (e) => {
    ...
    const fruitsandveg = {
      ...
      quantity:priceRef.current.value
    }
    try {
      const response = await addFruitsandveg(fruitsandveg)
  ...
  const editHelper = (fruitsandveg) => {
    console.log(fruitsandveg)
    setCurrentFruitsandveg(fruitsandveg)
    setShowEdit(true)
  }
  const handleEdit = async (e) => {
    ...
    try {
      const response = await editFruitsandveg(fruitsandveg, currentFruitsandveg._id)
      if (response.status === 200) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await deleteFruitsandveg(id)
      if (response.status === 200) {
  ...
  if (!fruitsandveg|| fruitsandveg.length === 0) {
    return (
      <>
        <div className='w-screen h-[90vh] flex flex-col justify-center items-center'>
  ...
  return (
    <div className='w-full flex flex-col justify-start items-start'>
      <div className='w-full flex flex-row justify-between items-center my-4 shadow-md rounded-md p-1 border'>
        <AdminPageHeader title='Fruitsandveg' />
      ...
      <table className='w-full h-full border-collapse border shadow-lg rounded-md'>
        ...
        <tbody>
          ...
          {
            fruitsandveg.map((fruitsandveg, index) => (
              <tr key={index}>
                <td className='p-4'>{fruitsandveg._id} </td>
                <td className='flex justify-start px-4 items-center'><img src={fruitsandveg.img} alt={fruitsandveg.title} className='h-12 w-12 object-cover rounded-full shadow-md bg-purple-500' /></td>
                <td className='p-4'>{fruitsandveg.title} </td>
                <td className='p-4'>{fruitsandveg.price}</td>
                <td className='p-4'>{fruitsandveg.quantity}</td>
               ...
               hover:bg-blue-500 hover:text-white hover:shadow-blue-500'
                    onClick={() => { editHelper(fruitsandveg) }}>
                    <Pencil />
                  </button>
               ...
               hover:bg-red-500 hover:text-white hover:shadow-red-500'
                    onClick={() => { handleDelete(fruitsandveg._id) }}>
                    <Trash />
                  </button>
                </td>
              </tr>
            ...
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  <h1 className='w-1/2 text-left text-xl mt-12 pt-20 font-bold text-green-500'>Add fruitsandveg</h1>
        ...
        <>
          <div className="absolute top-0 left-0 z-50 h-screen w-screen flex justify-center items-center bg-black/40 ">
            <div className='h-[55%] w-1/3 flex flex-col justify-center items-center bg-white shadow-2xl rounded-md'>
              <div className='h-full w-full flex flex-col justify-center items-center text-lg font-semibold'>
                <div className="h-[20%] w-[80%] flex flex-row justify-center items-center">
                  <h1 className='w-1/2 text-left text-xl mt-12 pt-20 font-bold text-blue-500'>Edit Fruitsandveg</h1>
                ...
                <form className='h-[70%] w-[80%] flex flex-col justify-center items-center gap-8' onSubmit={handleEdit}>
                  <input ref={titleRef} type="text" name="" id="title" placeholder='Title' defaultValue={currentFruitsandveg.title} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required autoFocus />
                  <input ref={imgRef} type="text" name="" id="img" placeholder='Image URL' defaultValue={currentFruitsandveg.img} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={priceRef} type="number" name="" id="price" placeholder='Price' defaultValue={currentFruitsandveg.price} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
                  <input ref={quantityRef} type="text" name="" id="quantity" placeholder='Quantity' defaultValue={currentFruitsandveg.quantity} className='w-full shadow-sm outline-none bg-[#f5f5f7] border-b-2 border-transparent p-4 focus:shadow-lg focus:border-b-2 focus:border-blue-400 rounded-sm' required />
...
export default AdminFruitsandveg